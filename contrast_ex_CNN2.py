...
def main():
        ...
        DNN_model.add(Conv2D(128, (1,8),strides=(1,4), data_format='channels_last',padding='valid', 
                         activation="relu", name="conv1", init='glorot_uniform'))

        DNN_model.add(Dropout(0.5))

        DNN_model.add(ZeroPadding2D((0,2)))
        DNN_model.add(Conv2D(80, (2,4),strides=(1,2), data_format='channels_last',padding='valid', 
        ...
        for snr in snrs:
                ...
                for i in range(0,len(classes)):
                        confnorm[i,:] = conf[i,:] / np.sum(conf[i,:])
                plt.figure()
                plot_confusion_matrix(confnorm, labels=classes, title="CNN2 Confusion Matrix (SNR=%d)"%(snr))
